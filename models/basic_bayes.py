...
MODEL_ATTRIB_DICT = {'ResNet' : {'args': ['num_classes', 'depth'], 
                                 'lin_layer': 'fc',
                                 'class': models.PreResNet},
                    'WideResNet' : {'args': ['num_classes', 'depth', 'widen_factor', 'dropout_rate'], 
                                    'lin_layer': 'linear', 
                                    'class': models.WideResNet},}
...
class BayesFCNet(Module):
    ...
    def __init__(**kwargs):
        ...
        if 'encoded' in kwargs['net_type']:
            ...
            base_init_class = MODEL_ATTRIB_DICT[archi_name]['class']
            base_init_args = MODEL_ATTRIB_DICT[archi_name]['args']
            linear_layer_name = MODEL_ATTRIB_DICT[archi_name]['lin_layer']
            
            base_init_dict = dict([(arg, kwargs[arg]) for arg in base_init_args if arg in kwargs])
            self.base_init(**base_init_dict)
            self.feature_size = self.__dict__[linear_layer_name].weight.size()[1]
            self.__dict__[linear_layer_name] = Identity()
        ...
        if len(fc_setup) > 0:
            ...
            if 'bayesian' not in self.net_type:
                ...
                for i in range(len(fc_setup)):
                    ...
                    w_name = 'fc'+ str(i) + '_w'
                    b_name = 'fc'+ str(i) + '_b'
                    self.check_name_clash([w_name, b_name])
                ...
                for i in range(len(fc_setup)):
                    ...
                    self.check_name_clash([w_mu, b_mu, w_sigma, b_sigma])
                ...
                if 'rank' in self.net_type:
                    ...
                    for i in range(len(fc_setup)):
                        ...
                        self.check_name_clash([w_sigma_B, b_sigma_B])
    ...
    def extract_feature(self, x):
        ...
        return super(ModifiedResNet, self).forward(x)
